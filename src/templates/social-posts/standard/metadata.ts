import { TemplateField, IndustryType, PreviewSize } from '../../_core/types';

// Template metadata
export const metadata = {
  id: 'social-media-post',
  name: 'Social Media Post',
  description: 'Engaging social media template for multiple platforms',
  industry: 'marketing' as IndustryType,
  tags: ['social', 'marketing', 'digital', 'promotion'],
  icon: '📱',
  // Size options for this template
  previewSizes: [
    { id: 'instagram', name: 'Instagram (Square)' },
    { id: 'instagramStory', name: 'Instagram Story' },
    { id: 'facebook', name: 'Facebook' },
    { id: 'twitter', name: 'Twitter' },
    { id: 'linkedin', name: 'LinkedIn' }
  ] as PreviewSize[],
  // Print configuration - social media posts only support PNG export
  printConfig: {
    formats: ['png'] as Array<'pdf' | 'png'>
  }
};

// Template fields schema
export const fields: TemplateField[] = [
  {
    id: 'headline',
    label: 'Headline',
    type: 'text',
    default: 'Introducing Our New Service',
  },
  {
    id: 'subtext',
    label: 'Subtext',
    type: 'text',
    default: 'Learn how we can help you achieve your goals',
  },
  {
    id: 'callToAction',
    label: 'Call to Action',
    type: 'text',
    default: 'Learn More',
  },
  {
    id: 'brandColor',
    label: 'Brand Color',
    type: 'color',
    default: '#ff5722',
  },
  {
    id: 'logo',
    label: 'Logo',
    type: 'image',
    default: `data:image/svg+xml;base64,`,
  },
  {
    id: 'backgroundImage',
    label: 'Background Image',
    type: 'image',
    default: '',
  }
];