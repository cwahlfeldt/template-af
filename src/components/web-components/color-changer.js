...
class ColorChanger extends HTMLElement {
    ...
    constructor() {
        ...
        this.#hiddenColorInput = this.shadowRoot.getElementById('color-picker-hidden');
    }
    ...
    set color(newValue) {
        ...
        const colorForInput = stringValue === 'transparent' ? '#ffffff' : stringValue;

        if (this.#currentColor !== stringValue) {
            this.#currentColor = stringValue;

            if (!this.#isProgrammaticChange && this.getAttribute('color') !== stringValue) {
    ...
    connectedCallback() {
        ...
        requestAnimationFrame(() => {
            ...
            if (!this.#slottedElement) {
                console.error("ColorChanger: No suitable element found in the default slot.", this);
            ...
            if (this.hasAttribute('persist')) {
                ...
                if (storageKey) {
                    try {
                        ...
                        console.warn(`ColorChanger (${this.id || 'no-id'}): Failed to read localStorage.`, e);
                    }
                } else if (!this.hasWarnedAboutMissingIdOnSave) {
                     console.warn(`ColorChanger: 'persist' attribute enabled but 'id' is missing. Cannot load from localStorage.`);
            ...
            this.#applyVisualUpdates(initialColor, initialColor === 'transparent' ? '#ffffff' : initialColor);
            ...
            console.debug(`ColorChanger (${this.id || 'no-id'}): Initialized (color source: ${sourceName}).`);
        });
    }
    ...
    attributeChangedCallback(name, oldValue, newValue) {
        ...
        console.debug(`ColorChanger Attribute Changed: ${name}, Old: ${oldValue}, New: ${newValue}`);
        ...
        switch (name) {
            ...
            case "id":
                ...
                // ---
                if (this.persist && oldValue !== null) { // Handle storage key change if persisting
                  console.warn(`ColorChanger: 'id' changed from '${oldValue}' to '${newValue}'. localStorage key will change.`);
    ...
    #updateHiddenInputValue(colorForInput) {
         if (this.#hiddenColorInput && this.#hiddenColorInput.value !== colorForInput) {
            try {
                this.#hiddenColorInput.value = colorForInput;
            } catch (e) {
                console.warn(`ColorChanger: Could not set hidden color input value to "${colorForInput}". Resetting to black.`, e);
    ...
    #saveToLocalStorage() {
        ...
        if (storageKey) {
            try {
                ...
                console.warn(`ColorChanger (${this.id || 'no-id'}): Failed to write localStorage ('${storageKey}').`, e);
            }
        } else if (this.persist && !this.hasWarnedAboutMissingIdOnSave) {
            console.warn(`ColorChanger: Cannot persist color without an 'id' attribute.`);
            this.hasWarnedAboutMissingIdOnSave = true;
        }
    }
    ...
    #clearFromLocalStorage() {
        ...
        if (storageKey) {
            try {
                ...
                console.debug(`ColorChanger (${this.id || 'no-id'}): Removed color from localStorage ('${storageKey}').`);
            } catch (e) {
                console.warn(`ColorChanger (${this.id || 'no-id'}): Failed to remove localStorage item ('${storageKey}').`, e);
            }
        }
    }
    ...
    #attachEventListeners() {
        ...
        this._boundHandleHiddenInputChange = this.#handleHiddenInputChange.bind(this);
        ...
        this.#hiddenColorInput?.addEventListener('change', this._boundHandleHiddenInputChange);
        this.#hiddenColorInput?.addEventListener('input', this._boundHandleHiddenInputInput);

        console.debug(`ColorChanger (${this.id || 'no-id'}): Event listeners attached.`);
    }

    #removeEventListeners() {
        this.#colorButton?.removeEventListener('click', this._boundHandleButtonClick);
        this.#hiddenColorInput?.removeEventListener('change', this._boundHandleHiddenInputChange);
        this.#hiddenColorInput?.removeEventListener('input', this._boundHandleHiddenInputInput);

        this._boundHandleButtonClick = null;
        this._boundHandleHiddenInputChange = null;
        this._boundHandleHiddenInputInput = null;

        console.debug(`ColorChanger (${this.id || 'no-id'}): Event listeners removed.`);
    }
    ...
    #observeMutations() {
        ...
        console.debug(`ColorChanger (${this.id || 'no-id'}): MutationObserver attached.`);
    }
        ...
        for (const mutation of mutationsList) {
            if (mutation.type === 'childList') {
                 ...
                 if (!this.#slottedElement && mutation.addedNodes.length > 0) {
                    const newElement = this.querySelector(':scope > *:not(style):not(script)');
                    if (newElement) {
                        console.log("ColorChanger: New slotted element detected.", newElement);
                        this.#slottedElement = newElement;
                        this.#applyVisualUpdates(this.#currentColor, this.#currentColor === 'transparent' ? '#ffffff' : this.#currentColor);
                        this.disabled = false;
    ...
    #applyColorToSlottedElement(colorToApply) {
        if (this.#slottedElement) {
            try {
                ...
                console.error(`ColorChanger: Failed to apply background color "${colorToApply}".`, e);
            }
        }
    }
    ...
    #updateIconAndGlobalContrastColor() {
        if (!this.#isInitialized) { // Don't run before init
             this.#currentContrastColor = '#000000'; // Ensure default
             this.style.setProperty('--color-changer-icon-color', this.#currentContrastColor);
        ...
        if (this.#slottedElement) {
            ...
            if (computedBgColor && computedBgColor !== 'transparent' && !computedBgColor.startsWith('rgba(0, 0, 0, 0)')) {
                try {
                    calculatedContrastColor = getContrastingTextColor(computedBgColor);
                } catch (e) {
                    console.error("ColorChanger: Error calculating contrast color. Using default.", e);
    ...
    #updateGlobalContrastVariable() {
        ...
        if (currentId) {
            ...
            try {
                document.documentElement.style.setProperty(globalVarName, this.#currentContrastColor);
                // console.debug(`ColorChanger: Set global var ${globalVarName} to ${this.#currentContrastColor}`);
            } catch (e) {
                console.error(`ColorChanger: Failed to set global CSS variable ${globalVarName}.`, e);
            }
    ...
    #removeGlobalContrastVariable(idToRemove) {
        if (idToRemove) {
            ...
            try {
                ...
                console.error(`ColorChanger: Failed to remove global CSS variable ${globalVarName}.`, e);
            }
        }
    }
    ...
    #handleButtonClick(event) {
        ...
        try {
            const currentVal = this.#currentColor === 'transparent' ? '#ffffff' : this.#currentColor;
            this.#updateHiddenInputValue(currentVal);
            this.#hiddenColorInput.click();
        } catch (e) {
             console.error("ColorChanger: Could not programmatically click hidden color input.", e);
        }
    }
    ...
    #handleHiddenInputChange(event) {
        ...
        console.debug(`ColorChanger Final Change: New color selected "${finalColor}".`);
    ...
    #dispatchChangeEvent(colorValue) {
        ...
        console.debug(`ColorChanger Dispatched 'change' event with color "${colorValue}".`);
    }
}
